...
1. Setup and prtitioning which would include the boot-strapping
   the install of the Root file system.

2. Chroot environemt during installation where basic packages are installed
   and the bootloader is configured. This also includes the creation
   of a user.

3. This is the time when the other system packages are installed along with
   the desktop environment.

4. Finally tunining of the system and installation of the non-default drivers.
...
```
Once you are statisfied, exit the `iw` shell.
```shell {linenos=false}
[iwd]# exit
```
...
```
If you get the ok result then you have successfuly connected to Internet.
...
ip a
```
### 5. (Optional) Setup NTP for Time and Date syncronization
...
cfdisk /dev/sda
```
Perform the Parition to obtain correct sizes of *Root* and *home*
paritions.

Confirm if the paritions are written to disk :
```shell {linenos=false}
lsblk
```
...
```
Create the directory for *home* paritions and mount it:
```shell {linenos=false}
mkdir /mnt/home
...
lsblk
```
### 10. Bootstrap the Instllation for Root File system
...
hwclock --systohc
```
You can verify that the clooks are indeed set right:
```shell {linenos=false}
date
```
...
passwd
```
Make sure to use 2 passwords one for **Root** and a diffrent one for the
...
```
### 7. Install the Bootloader and Configure
We would be using `GRUB` and its autogenerated configuration:
```shell {linenos=false}
...
## Stage 3 Commands
After we reboot, we can select `Arch Linx` in the `GRUB` menu.
This would drop us to the text login prompt.
Where we can login using `archer` user name and the password we 
set earlier.

### 1. Connect to the Internet
We would have to use a diffrent tool now `NetworkManager`:
```shell {linenos=false}
nmtui
```
...
cat /etc/pacman.d/mirrorlist
```
If we still find it incorect then run the `reflector` command again:
```shell {linenos=false}
reflector --country 'United States',Japan,India --age 6 \
 --protocol https --sort rate --save /etc/pacman.d/mirrorlist
```
...
Next we install the Desktop Enviroment and its utilities:
```shell {linenos=false}
...
```
In case you dont find the `fstrim` service just install
`sudo pacman -S util-linux`.
### 6. Adding some Essential XFCE utilites
Here is the sundry list of utilites:
```shell {linenos=false}
